...
# data
parser.add_argument('--path', type=str, default='../data/1.csv', help="文件路径")
parser.add_argument('--vision', type=bool, default=True)
parser.add_argument('--input_features', type=list, default=['time','U'], help="输入特征")
parser.add_argument('--output_features', type=list, default=['U'], help="输出特征")
...
one_scaled = one_scaler.fit_transform(data[['U']])
...
output_dir = Path(f'../{args.model_name}')
output_dir.mkdir(parents=True, exist_ok=True)# 在训练循环之后添加结果保存部分
current_time_str = time.strftime("%Y%m%d_%H%M%S")

# 保存训练集预测结果
train_results = pd.DataFrame({
    'timestamp': data['time'].iloc[args.window_size : args.window_size + len(y_train)],
    'actual': y_train_true.squeeze(),
    'predicted': y_train_pred_inv.squeeze()
})
train_results.to_csv(output_dir / f'train_results_{current_time_str}.csv', index=False)

# 保存验证集预测结果
val_results = pd.DataFrame({
    'timestamp': data['time'].iloc[args.window_size + len(y_train) : args.window_size + len(y_train) + len(y_val)],
    'actual': y_val_true.squeeze(),
    'predicted': y_val_pred_inv.squeeze()
})
val_results.to_csv(output_dir / f'val_results_{current_time_str}.csv', index=False)

# 保存测试集预测结果
test_results = pd.DataFrame({
    'timestamp': data['time'].iloc[args.window_size + len(y_train) + len(y_val) : args.window_size + len(y_train) + len(y_val) + len(y_test)],
    'actual': y_test_full.squeeze(),
    'predicted': y_test_pred_inv.squeeze()
})
test_results.to_csv(output_dir / f'test_results_{current_time_str}.csv', index=False)

# 保存自定义区间预测结果（如果有）
if 'y_custom_pred_inv' in locals():
    custom_results = pd.DataFrame({
        'timestamp': data['time'].iloc[args.predict_start : args.predict_end + 1],
        'actual': y_custom_true.squeeze(),
        'predicted': y_custom_pred_inv.squeeze()
    })
    custom_results.to_csv(output_dir / f'custom_{args.predict_start}-{args.predict_end}_results_{current_time_str}.csv', index=False)
...
metrics.to_csv(output_dir / f'metrics_{current_time_str}.csv', index=False)

# 保存完整配置
with open(output_dir / f'config_{current_time_str}.txt', 'w') as f:
    for key, value in vars(args).items():
        f.write(f"{key}: {value}\n")

print("所有预测结果和评估指标已保存至:", output_dir)